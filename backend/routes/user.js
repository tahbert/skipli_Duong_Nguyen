...
router.post('/:id', async (req, res) => {
    try {
        ...
        if (!favorite_github_users) {
            ...
            return res.status(200).send({ status: 200, message: 'successfully created field' })
        }
        if (favorite_github_users.some(user => user.id == likedUser.id)) {
            const response = await userRef.update({
                favorite_github_users: favorite_github_users.filter(user => user.id != likedUser.id)
            })
            return res.status(200).send({ status: 200, message: 'successfully disliked' })
        }
        const response = await userRef.update({
